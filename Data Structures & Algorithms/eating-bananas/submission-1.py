class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # Save result var to the largest pointer, in case the rate is the largest value
        # We'd never reach the largest number since the loop stops at l <= r
        res = r

        # Do binary search
        while l <= r:
            
            # Get middle candidate rate
            mid =(l + r) // 2
            ...
            # If the time is smaller, its valid
            # Save the result
            # update r and l
            if hours <= h:
                res = mid
