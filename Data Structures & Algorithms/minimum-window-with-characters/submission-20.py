class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        shortest = 100001           # Define the shortest length 
        curr_dict = {}              # Define current dict with all required chars
        l = 0                       # left pointer
        best_l = 0                  # Best l pointer
        ...
        for r in range(len(s)):
            ...
            if s[r] in t_dict:
                ...
                # If already satisfied, and want to check if shrinking can occur
                if all(curr_dict.get(c, 0) >= t_dict[c] for c in t_dict):

                    # Loop and Shrink until you reach a letter that can't be shrunk
                    while True:
                        # Move l by 1 if s[l] isn't in t_dict
                        if not s[l] in t_dict:
                            l += 1
                    ...
                    # Only update if this window is shorter
                    if r - l + 1 < shortest:
                        best_l = l
                        best_r = r
                        shortest = r - l + 1
